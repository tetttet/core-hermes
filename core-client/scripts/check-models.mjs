...
const requestedModelIds = new Set(process.argv.slice(2));
const apiModels = MODELS.filter(
  (model) =>
    model.id !== AUTO_MODEL_ID &&
    (requestedModelIds.size === 0 || requestedModelIds.has(model.id)),
);
const imageModel = apiModels.find((model) => model.supportsVision);
const videoModel = apiModels.find((model) => model.supportsVideo);
...
if (imageModel && process.env.MODEL_CHECK_IMAGE_PATH) {
...
if (videoModel && process.env.MODEL_CHECK_VIDEO_PATH) {
